...
class WorldManager extends \pocketmine\plugin\PluginBase implements \pocketmine\event\Listener{
	...
	public function onEntityDamage(\pocketmine\event\entity\EntityDamageEvent $event){
		if(!$event->isCancelled() && ($entity = $event->getEntity()) instanceof Player && $event->getCause() <= 11 && $event instanceof \pocketmine\event\entity\EntityDamageByEntityEvent && !in_array(strtolower($entity->getLevel()->getFolderName()), $this->wm["PVP"]) && ($damager = $event->getDamager()) instanceof Player && !$damager->hasPermission("worldmanager.worldpvp.pvp")){
			$event->setCancelled();
//			$damager->sendMessage(Color::RED . "[PVP Manager] PVP 권한이 없습니다.");
		}
	}
	...
	public function loadWorlds(){
		...
		foreach($this->wm["Spawn"] as $levelName => $spawn){
			if($level = $this->getLevelByName($levelName)){
				$level->setSpawnLocation(new \pocketmine\level\Position(...(explode(":", $spawn) + [$level])));
			}
		}
	}
