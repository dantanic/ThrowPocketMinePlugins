...
class CocoaBean extends \pocketmine\plugin\PluginBase implements \pocketmine\event\Listener{
	...
	public function onEntityDamage(EntityDamageEvent $event){
		...
		if($event->getCause() == 3){
			for($x = -1; $x < 2; $x++){
				for($z = -1; $z < 2; $z++){
					if($e->getLevel()->getBlockIdAt(round($e->x) + $x, round($e->y + 1), round($e->z + $z)) == 127){
						$event->setCancelled();
						break;
					}
				}
	...
	public function getPos($b){
		return $b->x . ":" . $b->y . ":" . $b->z . ":" . $b->getLevel()->getName();
	}
	...
	public function loadBeanYml(){
		@mkdir($this->getDataFolder());
		$this->cb = (new Config($this->getDataFolder() . "CocoaBean.yml", Config::YAML, []))->getAll();
	}

	public function saveBeanYml(){
		$cb = [];
		foreach($this->grow as $k => $v){
			$cb[$k] = $v->getDamage();
		}
		ksort($cb);
		$cbc = new Config($this->getDataFolder() . "CocoaBean.yml", Config::YAML, []);
		$cbc->setAll($cb);
		$cbc->save();
		$this->cb = $cb;
	}
