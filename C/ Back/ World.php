...
class World implements Plugin{
	...
	public function init(){
		$this->api->console->register("worlds", "<command> [parameters...]", array($this, "command"));
		$this->config = new Config($this->api->plugin->configPath($this)."config.yml", CONFIG_YAML, array(
			"default-generator" => "SuperflatGenerator",
			"autogenerate" => false,
			"autoload" => array("gs","world"),
		));
	...
	public function generateLevel($name, $seed, $generator = false){
		if($this->api->level->levelExists($name)) return false;
		if($generator === false) $generator = $this->config->get("default-generator");
