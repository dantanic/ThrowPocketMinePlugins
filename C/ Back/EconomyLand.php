...
class EconomyLand implements Plugin {
	...
	public function init(){
		...
		//$kcmd = array( "시작점" => "", "끝점" => "", "땅" => "<목록 | 위치 | 이동 | 초대 | 초청객 | 주기> [페이지 | 땅 번호] [(r:)플레이어]", "땅팔기" => "<여기 | 땅 번호>");
	...
	public function commandHandler($cmd, $param, $issuer, $alias){
		...
		case "landsell":
			...
			case "here":
				...
				if($info["owner"] !== $issuer->username){
					...
					$this->land->exec("DELETE FROM land WHERE ID = {$result["ID"]}");
				}
				break;
			default:
				$p = $param[0];
				if(is_numeric($p)){
					$result = $this->land->query("SELECT * FROM land WHERE ID = $p");
					$info = $result->fetchArray(SQLITE3_ASSOC);
					if(is_bool($info)){
		...
		case "land":
			...
			if($p == "move"){
				...
				if(is_numeric($this->config["player-land-limit"]) and $cnt >= $this->config["player-land-limit"]){
