	...
	public function onCommand(CommandSender $sender, Command $cmd, $label, array $sub){
		...
		switch(strtolower($sub[0])){
			...
			case "추가":
				...
				}elseif(!isset($sub[5]) || $sub[1] == "" || $sub[2] == "" || $sub[3] == "" || $sub[4] == "" || $sub[5] == ""){
					...
					if(!preg_match(self::REGEX_ID, $sub[1], $idMatch)){
						...
						if($item->getID() == 0){
							...
							if(!($isPlayer = $sender instanceof Player) || $isPlayer && isset($sub[8]) && $sub[6] != "" | $sub[7] != "" | $sub[8] != ""){
								...
								$this->editTouch[$name] = [self::TOUCH_MODE => self::MODE_ADD, self::ITEM_ID => $id, self::BUY_AMOUNT => $sub[2], self::BUY_PRICE => $sub[3], self::SELL_AMOUNT => $sub[4], self::SELL_PRICE => $sub[5]];
				...
				if($this->data[$pos][self::BUY_AMOUNT] == "-" && $this->data[$pos][self::SELL_AMOUNT] == "-"){
					...
					if($player->isCreative()){
						$player->sendMessage(Color::RED . "[OneShop] " . ($ik ? 
							...
							"You are creative mode.\n" . Color::DARK_RED . "  [OneShop][$pos] ID: " . $this->data[$pos][self::ITEM_ID] . ", Buy Amount: "  . (is_numeric($this->data[$pos][self::BUY_AMOUNT]) ? $this->data[$pos][self::BUY_AMOUNT] . ", " : ($this->data[$pos][self::BUY_AMOUNT] == "?" ? "All, " : $this->data[$pos][self::BUY_AMOUNT]) . ", Unit") . " Price: " . $this->data[$pos][self::BUY_PRICE] . "$, Sell Amount: "  . (is_numeric($this->data[$pos][self::SELL_AMOUNT]) ? $this->data[$pos][self::SELL_AMOUNT] . ", " : ($this->data[$pos][self::SELL_AMOUNT] == "?" ? "All, " : $this->data[$pos][self::SELL_AMOUNT]) . ", Unit") . " Price: " . $this->data[$pos][self::SELL_PRICE] . "$"
						));
					}else{
							...
							$player->sendMessage(Color::RED . "[OneShop] " . ($ik ? "천천히 눌러주시기바랍니다." : "Please slow down"));
		 					...
		 					if($shopMode == self::MODE_SELL){
		 						if($this->data[$pos][self::SELL_AMOUNT] == "-"){
									...
									$player->sendMessage(Color::YELLOW . "[OneShop] " . ($ik ? 
										"판매하시려면 다시 눌러주세요.\n" . Color::YELLOW . "  [OneShop] 아이디: " . $this->data[$pos][self::ITEM_ID] . ", 갯수: "  . (is_numeric($this->data[$pos][self::SELL_AMOUNT]) ? $this->data[$pos][self::SELL_AMOUNT] . ", " : ($this->data[$pos][self::SELL_AMOUNT] == "?" ? "모두, " : $this->data[$pos][self::SELL_AMOUNT]) . ", 개당") . " 가격: " . $this->data[$pos][self::SELL_PRICE] . "$, 소유갯수: $count" : 
										"Touch twice to sell item.\n" . Color::YELLOW . "  [OneShop] ID: " . $this->data[$pos][self::ITEM_ID] . ", Amount: "  . (is_numeric($this->data[$pos][self::SELL_AMOUNT]) ? $this->data[$pos][self::SELL_AMOUNT] . ", " : ($this->data[$pos][self::SELL_AMOUNT] == "?" ? "All, " : $this->data[$pos][self::SELL_AMOUNT]) . ", Unit") . " Price: " . $this->data[$pos][self::SELL_PRICE] . "$, You have: $count"
	 								));
	 							}
							}else{
		 						if($this->data[$pos][self::BUY_AMOUNT] == "-"){
									...
									$player->sendMessage(Color::YELLOW . "[OneShop] " . ($ik ? 
										"구매하시려면 다시 눌러주세요.\n" . Color::YELLOW . "  [OneShop] 아이디: " . $this->data[$pos][self::ITEM_ID] . ", 갯수: "  . (is_numeric($this->data[$pos][self::BUY_AMOUNT]) ? $this->data[$pos][self::BUY_AMOUNT] . ", " : ($this->data[$pos][self::BUY_AMOUNT] == "?" ? "모두, " : $this->data[$pos][self::BUY_AMOUNT]) . ", 개당") . " 가격: " . $this->data[$pos][self::BUY_PRICE] . "$, 나의 돈: $money, 최대 구매량: $maxAmount" : 
										"Touch twice to buy item.\n" . Color::YELLOW . "  [OneShop] ID: " . $this->data[$pos][self::ITEM_ID] . ", Amount: "  . (is_numeric($this->data[$pos][self::BUY_AMOUNT]) ? $this->data[$pos][self::BUY_AMOUNT] . ", " : ($this->data[$pos][self::BUY_AMOUNT] == "?" ? "All, " : $this->data[$pos][self::BUY_AMOUNT]) . ", Unit") . " Price: " . $this->data[$pos][self::BUY_PRICE] . "$, Your money: $money, Max Amount: $maxAmount"
									));
								}
							}
 						}else{
