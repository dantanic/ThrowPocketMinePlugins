...
class MineFarm extends PluginBase implements Listener{
	...
	public function onEnable(){
		...
		$s->getScheduler()->scheduleRepeatingTask(new CallbackTask([$this, "onTick"]), 20);
	...
	public function onPlayerInteract(PlayerInteractEvent $event){
		if($event->isCancelled() || $event->getFace() == 255) return;
		$p = $event->getPlayer();
		if($p->hasPermission("minefarm.block")) return;
		$b = $event->getBlock();
		$i = $event->getItem();
		if($this->isFarm($b)){
	...
	public function onBlockPlace(BlockPlaceEvent $event){
		if($this->isFarm($b = $event->getBlock()) && !$this->isOwn($p = $event->getPlayer(), $b) && !$this->isShare($p, $b) && !$p->hasPermission("minefarm.block")) $event->setCancelled();
	}

	public function onBlockUpdate(BlockUpdateEvent $event){
		if($this->isFarm($b = $event->getBlock()) && $this->isMain($b) && $b->y < 8 && in_array($b->getID(), [8, 9, 10, 11])) $event->setCancelled();
	}
