...
class ItemCase extends PluginBase implements Listener{
	...
	public function onBlockEvent($event, $isTouch = false){
		...
		if(isset($this->ic[$this->getPos($event->getBlock())])){
			...
			if(!$p->hasPermission("itemcase.block")) $event->setCancelled();
			if($p->hasPermission("itemcase.spawn")) $this->spawnCase(true);
		}
	}
	...
	public function spawnCase($time = false){
		...
		foreach($this->ic as $k => $list){
			...
			foreach($list as $item => $v){
				...
				foreach($this->getServer()->getOnlinePlayers() as $p){
					if($p->getLevel()->getFolderName() == $pos[3]) $p->addEntityMovement($this->eid, $pos[0] + 0.5, $pos[1] + 0.25, $pos[2] + 0.5, 0, 0, 0);
				}
	...
	public function loadYml(){
		...
		$this->ic = (new Config($this->getDataFolder() . "ItemCase.yml", Config::YAML))->getAll();
	}

	public function saveYml(){
		ksort($this->ic);
		$ic = new Config($this->getDataFolder() . "ItemCase.yml", Config::YAML);
		$ic->setAll($this->ic);
		$ic->save();
	}
