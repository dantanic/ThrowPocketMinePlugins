...
class Money extends PluginBase implements Listener{
		...
		switch(strtolower($cmd->getName())){
				...
				switch(strtolower($sub[0])){
					...
					case "지불":
						if(!$sender instanceof Player){
							...
							$r = $rm . "Pay(P) " . ($ik ? "<플레이어명> <돈> " : "<PlayerName> <Money>");
							...
							$r = $mm . ($ik ? "돈이 $sub[2] 보다 부족합니다." : "You don't have $sub[2]");
						}else{
			...
			case "moneyop":
				...
				switch(strtolower($sub[0])){
					...
					case "설정":
						if(!isset($sub[1])){
							$r = $rm . "Set(S) " . ($ik ? "<플레이어명> <돈>" : "<PlayerName> <Money>");
						}elseif(!($p = $this->getPlayer($sub[1]))){
							$r = $mm . $sub[1] . ($ik ? "은 잘못된 이름입니다." : " is invalid name");
					...
					case "지급":
						if(!isset($sub[1])){
							$r = $rm . "Give(G)" . ($ik ? "<플레이어명> <돈>" : "<PlayerName> <Money>");
						}elseif(!($p = $this->getPlayer($sub[1]))){
							$r = $mm . $sub[1] . ($ik ? "은 잘못된 이름입니다." : " is invalid name");
						}elseif(!is_numeric($sub[2]) || $sub[2] < 0){
							$r = $mm . $sub[2] . ($ik ? "은 잘못된 숫자입니다." : " is invalid number");
					...
					case "뺏기":
						if(!isset($sub[1])){
							$r = $rm . "Take(T) " .  ($ik ? "<플레이어명> <돈>" : "<PlayerName> <Money>");
						}elseif(!($p = $this->getPlayer($sub[1]))){
							$r = $mm . $sub[1] . ($ik ? "은 잘못된 이름입니다." : " is invalid name");
						}elseif(!is_numeric($sub[2]) || $sub[2] < 0){
							$r = $mm . $sub[2] . ($ik ? "은 잘못된 숫자입니다." : " is invalid number");
	...
	public function loadYml(){
		...
		$this->mn = (new Config($this->getDataFolder() . "Money.yml", Config::YAML, ["Money" => [], "Default" => "10000", "OP" => true]))->getAll();
	}

	public function saveYml(){
		asort($this->mn);
		$mn = new Config($this->getDataFolder() . "Money.yml", Config::YAML);
		$mn->setAll($this->mn);
		$mn->save();
	}
