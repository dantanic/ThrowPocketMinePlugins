...
class Shop extends \pocketmine\plugin\PluginBase implements \pocketmine\event\Listener{
	...
	public function onLoad(){
		...
		if(!($this->money = $pm->getPlugin("PocketMoney")) && !($this->money = $pm->getPlugin("EconomyAPI")) && !($this->money = $pm->getPlugin("MassiveEconomy")) && !($this->money = $pm->getPlugin("Money"))){
			...
			$this->MovePlayerPacket = new \pocketmine\network\protocol\MovePlayerPacket();
	 		$this->touch = $this->tap = $this->place = $this->players = $this->eids = [];
		}
	}
		...
		switch(strtolower($sub[0])){
			...
			case "추가":
				if(isset($t[$name = $sender->getName()])){
					$r = Color::YELLOW . "[Shop] " . ($ik ? "상점 추가 해제" : " Shop Add Touch Disable");
					unset($t[$name]);
				}else{
					if(!isset($sub[4])){
			...
			case "제거":
				if(isset($t[$name = $sender->getName()])){
					$r = Color::YELLOW . "[Shop] " . ($ik ? "상점 제거 해제" : " Shop Del Touch Disable");
					unset($t[$name]);
				}else{
					$r = Color::YELLOW . "[Shop] " . ($ik ? "대상 블럭을 터치해주세요. " : "Touch the block");
					$t[$name] = ["Type" => "Del"];
				}
			break;
			case "reset":
	...
	public function onBlockBreak(\pocketmine\event\block\BlockBreakEvent $event){
		if(isset($this->sh[$this->getPos($event->getBlock())])){
			...
			if($event->getItem()->isPlaceable()) $this->place[$player->getName()] = true;
			if(!$player->hasPermission("shop.block")) $event->setCancelled();
		}
	}
	...
	public function onPlayerInteract(\pocketmine\event\player\PlayerInteractEvent $event){
		...
		if(isset($t[$name = $player->getName()])){
			switch($t[$name]["Type"]){
				case "Add":
