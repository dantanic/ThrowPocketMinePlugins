...
class DamageBlock extends PluginBase{
		...
		switch(strtolower($sub[0])){
			...
			case "추가":
				if(!isset($sub[1]) || !isset($sub[2])){
					$r = $rm . "Add(A) " . ($ik ? "<블럭ID> <데미지1> <데미지2>" : "<BlockID> <Damage1> <Damage2>");
				}else{
	...
	public function onTick(){
		...
		foreach($this->getServer()->getOnlinePlayers() as $p){
			...
			for($z = $minZ; $z <= $maxZ; ++$z){
				for($x = $minX; $x <= $maxX; ++$x){
					for($y = $minY; $y <= $maxY; ++$y){
						$getDamage = $this->getDamage($b = $p->getLevel()->getBlock(new Vector3($x, $y, $z)));
			...
			if($damage !== 0) $p->attack($damage, new EntityDamageByBlockEvent($b, $p, EntityDamageEvent::CAUSE_CONTACT, 1));
	...
	public function loadYml(){
		...
		$this->db = (new Config($this->getDataFolder() . "DamageBlock.yml", Config::YAML, []))->getAll();
	}

	public function saveYml(){
		ksort($this->db);
		$db = new Config($this->getDataFolder() . "DamageBlock.yml", Config::YAML, []);
		$db->setAll($this->db);
		$db->save();
	}
