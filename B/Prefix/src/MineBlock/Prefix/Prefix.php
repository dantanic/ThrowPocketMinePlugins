...
class Prefix extends PluginBase{
	...
	public function loadYml(){
		...
		$this->pf = (new Config($this->getDataFolder() . "Prefix.yml", Config::YAML, ["Format" => "[%prefix] %name", "Default" => "User", "Player" => []]))->getAll();
	}

	public function saveYml(){
		ksort($this->pf);
		$pf = new Config($this->getDataFolder() . "Prefix.yml", Config::YAML, []);
		$pf->setAll($this->pf);
		$pf->save();
	}
