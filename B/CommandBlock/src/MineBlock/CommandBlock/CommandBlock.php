...
class CommandBlock extends PluginBase implements Listener{
	...
	public function onPlayerInteract(PlayerInteractEvent $event){
		...
		if(isset($t[$n])){
			...
			$this->touch = $t;
			$event->setCancelled();
			if($event->getItem()->isPlaceable()) $this->place[$p->getName()] = true;
		}else{
	...
	public function onBlockEvent($event, $isTouch = false){
		...
		if(isset($this->cb[$pos])){
			if($isTouch && $event->getItem()->isPlaceable()) $this->place[$p->getName()] = true;
	...
	public function runCommand($p, $pos, $isBlock = false){
		...
		foreach($cb[$pos] as $str){
			$arr = explode(" ", str_ireplace(["%player", "%p", "%x", "%y", "%z", "%blockx", "%bx", "%blocky", "%by", "%blockz", "%bz", "%world", "%w", "%server", "%s", "%version", "%v", "%money", "%m"], [$p->getName(), $p->getName(), $p->x, $p->y, $p->z, $l[0], $l[0], $l[1], $l[1], $l[2], $l[2], $p->getLevel()->getFolderName(), $p->getLevel()->getFolderName(), $this->getServer()->getServerName(), $this->getServer()->getServerName(), $this->getServer()->getApiVersion(), $this->getServer()->getApiVersion(), ($money = $this->getMoney($p)) !== false ? $money : 0, ($money = $this->getMoney($p)) !== false ? $money : 0], $str));
			$time = 0;
			$chat = false;
			$console = false;
			$op = false;
			...
			$this->getServer()->getScheduler()->scheduleDelayedTask(new Task($this, [$this,"dispatchCommand"], [$p,$pos,$isBlock,$chat,$console,$op,$deop,$safe,$block,$arr,$heal,$damage,$say]), $time * 20);
		}
	...
	public function loadYml(){
		...
		$this->cb = (new Config($this->getDataFolder() . "CommandBlock.yml", Config::YAML))->getAll();
	}

	public function saveYml(){
		ksort($this->cb);
		$cb = new Config($this->getDataFolder() . "CommandBlock.yml", Config::YAML);
		$cb->setAll($this->cb);
		$cb->save();
	}
