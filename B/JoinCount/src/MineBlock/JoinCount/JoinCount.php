...
class JoinCount extends PluginBase implements Listener{
	...
	public function onCommand(CommandSender $sender, Command $cmd, $label, array $sub){
		...
		switch(strtolower($sub[0])){
			...
			case "보기":
				if(!isset($sub[1])){
					$r = $rm . "Veiw(V) " . ($ik ? "<플레이어명>" : "<PlayerName>");
	...
	public function onPlayerJoin(PlayerJoinEvent $event){ //접속할때마다 접속횟수 +1
		$this->joinLog($event);
	}
	...
	public function loadYml(){
		...
		$this->jc = (new Config($this->getDataFolder() . "JoinCount.yml", Config::YAML))->getAll();
	}

	public function saveYml(){
		asort($this->jc);
		$jc = new Config($this->getDataFolder() . "JoinCount.yml", Config::YAML);
		$jc->setAll($this->jc);
		$jc->save();
	}
